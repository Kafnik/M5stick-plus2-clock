...
def draw_ui_design(status_idx=0):
    ...
    lcd.print("M5STACK STUDIO v4.4", 5, 4, 0x00CCFF)
...
while True:
    ...
    if time.ticks_diff(time.ticks_ms(), last_activity) > TIMEOUT:
        # Просто гасим экран и выключаем
        lcd.setBrightness(0)
        try:
            import axp
            axp.powerOff() # Попытка через контроллер питания
        except:
            machine.reset() # Если не выключился, хотя бы перезагрузимся
